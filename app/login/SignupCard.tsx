...
type SignupCardProps = { plan: TeacherPlan; onCompleteChange?: (complete: boolean) => void };

export async function payTeacherPlan(paymentMethod: string, auth: SignupPaymentAuth, selectedPlan: TeacherPlan, onTotal?: (total: string) => void) {
  const key = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;
  const stripe = key ? await loadStripe(key) : null;
  if (!stripe) throw new Error("Card payments are temporarily unavailable. Please try again later.");
  ...
  onTotal?.(session.total.total.amount);
...
const CardFields = forwardRef<SignupCardHandle, SignupCardProps>(function CardFields({ plan, onCompleteChange }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    async collect(email) {
      ...
      const result = await stripe.createPaymentMethod({ type: "card", card, ...(email ? { billing_details: { email } } : {}) });
    ...
    async pay(paymentMethod, auth, selectedPlan) {
      await payTeacherPlan(paymentMethod, auth, selectedPlan, nextTotal => flushSync(() => setTotal(nextTotal)));
    },
  ...
  return <section aria-label="Credit card payment" className="space-y-2">
    ...
    <div className="rounded-xl border border-slate-300 bg-white px-4 py-4">
      <CardElement options={{ style: { base: { fontSize: "16px", color: "#0f172a", "::placeholder": { color: "#94a3b8" } }, invalid: { color: "#b91c1c" } } }} onReady={() => setReady(true)} onChange={event => { setError(event.error?.message || ""); onCompleteChange?.(event.complete); }} />
    </div>
...
export default forwardRef<SignupCardHandle, SignupCardProps>(function SignupCard({ plan, onCompleteChange }, ref) {
  const key = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;
  ...
  return <Elements stripe={stripe}><CardFields ref={ref} plan={plan} onCompleteChange={onCompleteChange} /></Elements>;
});
